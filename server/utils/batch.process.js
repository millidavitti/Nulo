...
function init(events) {
  ...
  events.on("hotels", async (cycle, url, db) => {
    ...
    events.emit(
      "countries",
      new FetchCycle(1),
      new Url("/locations/countries", 1, 10),
      countriesDB
    );
  });
  // Locations
  events.on("countries", async (cycle, url, db) => {
    ...
    events.emit(
      "destinations",
      new FetchCycle(7),
      new Url("/locations/destinations", 1, 10),
      destinationsDB
    );
  });
  ...
  events.on("boards", async (cycle, url, db) => {
    ...
    events.emit(
      "accommodations",
      new FetchCycle(1),
      new Url("/types/accommodations", 1, 10),
      accommodationsDB
    );
  });
  ...
  events.on("facilities", async (cycle, url, db) => {
    ...
    events.emit(
      "facilitygroups",
      new FetchCycle(1),
      new Url("/types/facilitygroups", 1, 10),
      facilitygroupsDB
    );
  });
  ...
  events.on("terminals", async (cycle, url, db) => {
    ...
    events.emit(
      "ratecomments",
      new FetchCycle(5),
      new Url("/types/ratecomments", 1, 10),
      ratecommentsDB
    );
  });
  ...
  events.emit("hotels", new FetchCycle(5), new Url("/hotels", 1, 10), hotelsDB);
}
