...
/* harmony export */   "G5": () => (/* binding */ imgArray),
/* harmony export */   "KK": () => (/* binding */ cityImgs),
/* harmony export */   "P1": () => (/* binding */ bookedDays),
/* harmony export */   "Ze": () => (/* binding */ generateCalendar),
/* harmony export */   "mM": () => (/* binding */ stays)
/* harmony export */ });
/* unused harmony export rand */
function generateCalendar(month, year) {
    const calendar = [];
    const monthStart = new Date(year, month).getDay();
    /*prettier-ignore */ const daysOfMonth = [
        31,
        getFebDays(year),
        31,
        30,
...
function bookedDays(from, to) {
    //  Same month of the same year
    if (from?.year === to?.year && from?.month === to?.month) return to?.date - from?.date;
    //  diff months of the same year
    if (from?.year === to?.year && from?.month < to?.month && to?.month - from?.month < 2) {
    ...
    if (from?.year < to?.year) {
        ...
        for(let year = from?.year; year <= to?.year; year++){
            if (year === from?.year) for(let i1 = from?.month; i1 < 12; i1++){
                if (i1 === from?.month) result1 += tmd(year)[i1] - from?.date;
                else result1 += tmd(year)[i1];
            }
            else if (year === to?.year) for(let j = 0; j <= to?.month; j++){
                if (j === to?.month) result1 += to?.date;
                else result1 += tmd(year)[j];
            }
            else if (year > from?.year && year < to?.year) for(let k = 0; k < 12; k++){
                result1 += tmd(year)[k];
            }
        }
        return result1;
...
function isLeapYear(year) {
    return !(year % 4) && year % 100 && year % 400 || !(year % 100) && !(year % 400);
}
