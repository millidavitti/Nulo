...
// SSR
import { pollHotels, pollCountry } from '../../../hooks/request'
...
// Component
export default function Listings({ cityListings, country }) {
 const [state, dispatch] = useReducer(reducer, init)
 const { city: destinationName } = useRouter().query
 const destinationCountry = JSON.parse(country)?.description
 ...
 return (
  <>
    ...
    <Container className={listings.container}>
     <Grid className={listings.heroGrid}>
      <div className={listings.locationWrap}>
       ...
       <Headings fontSize="3rem">
        {destinationName}, {destinationCountry?.content}
       </Headings>
       <Margin topBottom={20}>
     ...
     <Container>
      ...
      <div className={listings.filters}>
       <div className={listings.deskFilters}>
        <button
         ...
         onClick={() => dispatch({ type: 'desk-filters', id: 'typeofPlace' })}
        >
         Type of place
        </button>
        <div
         ...
         style={{
          ...
          pointerEvents: state.deskFilters[0].typeofPlace ? 'unset' : 'none',
         }}
        >
       ...
       <div className={listings.deskFilters}>
        <button
         className={listings.morefiltersBtn}
         onClick={() => dispatch({ type: 'desk-filters', id: 'priceRange' })}
        >
         $0 -$1000
        </button>
        <div
         ...
         style={{
          ...
          pointerEvents: state.deskFilters[1].priceRange ? 'unset' : 'none',
         }}
        >
      ...
      <Suspense fallback={'Loading'}>
       <ResponsiveGrid className={listings.resListings}>
        {JSON.parse(cityListings).map((city, key) => (
         <HotelCard data={imgArray} city={city} key={key} />
        ))}
       </ResponsiveGrid>
      </Suspense>
   ...
   <div
    ...
    id="overlay"
    onClick={(e) => {
     if (e.target.closest('span')?.id === 'close' || e.target?.id === 'overlay')
     ...
     >
      ...
      <div>
       ...
       <div className={listings.roomsPickFilter}>
        <h3 className={listings.filterType}>Beds, Bedrooms and Bathroom</h3>
        <div className={listings.roomPicks}>
         {/* Beds */}
         <div className={listings.roomPick}>
          <div className={listings.guestPickLabel}>
...
export async function getServerSideProps({ params, query }) {
 const cityListings = await pollHotels(params.cityListings)
 console.log('ISO: ', query.isoCode)
 const country = await pollCountry(query.isoCode)
 return {
  props: { cityListings, country },
 }
}
